...
(function () {
	...
	var new_initPopupMenu = function (type, menuItems, x, y) {
		...
		if (currentZone === 0)
			if (GameManager.canDevelopConsole())
				menuItems.push({
					label: "Phát triển máy tính...".localize("menu item"),
					action: function () {
						curPage = 0;
						Sound.click();
						Tutorial.devConsole();
	...
	Media.createComputerStartStory = function (computer) {
		...
		var msg = "{0} đã phát hành máy tính {1} của họ hôm nay.".localize().format(company.name, computer.name);
		if (isGoodTech)
			msg += " " + "Máy tính này dường như thực sự vượt qua giới hạn công nghệ và là máy tính hiện đại nhất từng xuất hiện trên thị trường.".localize();
		else
			msg += " " + "Máy tính này dường như không hoàn toàn ngang tầm với các đối thủ công nghệ cao, nhưng chúng ta sẽ xem người dùng nghĩ gì.".localize();
		msg += " Nhìn vào các tính năng của {0}, có vẻ như danh sách ".localize().format(computer.name);
		if (featureFactor >=
			0.8)
			msg += "rất phong phú, đây là một dấu hiệu tốt và có thể dẫn đến nhiều loại game khác nhau sẽ được phát triển cho nó.".localize();
		else
			msg += " " + "hơi ít ỏi. Đừng mong đợi quá nhiều tiện ích và thiết bị sẽ có sẵn cho máy tính này.".localize();
		msg += "{n}" + "Các bài kiểm tra đầu tiên cho thấy {0}".localize().format(computer.name);
		if (quality >= 0.8)
			msg += " có chất lượng hoàn thiện xuất sắc và có khả năng chịu được áp lực, ngay cả từ những game nặng nhất của bạn.".localize();
		else if (quality >= 0.5)
			msg += " " + "có chất lượng hoàn thiện trung bình. Đừng mong đợi nó sẽ tồn tại mãi mãi nhưng nhìn chung bạn sẽ không gặp nhiều vấn đề.".localize();
		else
			msg += " " + "hơi mỏng manh. Chúng tôi sẽ không ngạc nhiên nếu bạn cần sử dụng bảo hành sớm hơn hoặc muộn hơn.".localize();
		msg += "\n" + "Nhìn chung ".localize("fragment continues with 'we think that the computer...'");
		if (success >= 1)
			msg += " " + "chúng tôi nghĩ rằng máy tính này sẽ làm xáo trộn thị trường và chứng tỏ rất thành công.".localize("fragment, started with 'All in all'").format(computer.name);
		else if (success >= 0.8)
			msg += " " + "chúng tôi nghĩ rằng máy tính này sẽ hoạt động khá tốt trên thị trường và là một sự bổ sung đáng hoan nghênh.".localize("fragment, started with 'All in all'").format(computer.name);
		else
			msg += " " + "thật khó để nói liệu máy tính này sẽ hoạt động tốt hay không vì có quá nhiều máy tính tốt khác trên thị trường.".localize("fragment, started with 'All in all'").format(computer.name);
		company.notifications.push(new Notification({
			header: "Tin Tức Ngành".localize("heading"),
			text: msg,
			weeksUntilFired: 0.4,
