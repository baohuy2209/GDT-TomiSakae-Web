...
$.each(b.css,function(f,g){var h=$(document.createElement("link"));
h.attr({rel:"stylesheet",type:"text/css",href:GDT.getRelativePath()+"/css/"+g+".css",media:"all"});
e.append(h);
});
...
a.addSection=function(b,c){b.append(c);
};
...
w.attr({id:"TweakModNotificationReplacement1"});
w.appendTo($("body"));
var j=1;
if(window.devicePixelRatio!==undefined){j=window.devicePixelRatio;
}var v=$(window).width()*j;
var u=$(window).height()*j;
var i=$(document);
var g=(v*0.5)-(230);
var h=(u*0.5)-(120);
w.css({position:"absolute",left:g,top:h,width:460,height:"auto",padding:5,backgroundColor:"#f0f0f0",opacity:"0.9",border:"4px solid rgb(255,209,123)",display:"none",zIndex:8000,boxShadow:"0 0 5px #888"});
var o=false;
var m=false;
switch(e.header){case"{ReleaseGame}":break;
case"{Reviews}":break;
default:var s=a.getItemByName(e.header);
if(s){b=true;
o=s.asOverlay;
m=s.enabled;
}break;
}if(m){if(o){var l="<h3>"+e.header+"</h3>";
l+=e.text.replace("\n","<br/><br/>");
w.html(l).delay(500).fadeIn().delay(4000).fadeOut();
UltimateLib.Elements.SimpleModalContainer.css({position:"absolute",left:"-100px"});
...
function a(i,k){i.id="UL-Research-"+i.id;
if(k===0){var e=i.canResearch;
i.canResearch=function(l){return l.flags.hwLabUnlocked===true&&e(l);
};
}if(i.repeatable===false){var f=i.complete;
var g=i.canResearch;
i.complete=function(l){GDT.getDataStore("UL-Research").data[this.id].complete=true;
f(l);
};
i.canResearch=function(l){var m=true;
try{if(GDT.getDataStore("UL-Research").data[this.id].complete===true){m=false;
}}catch(n){if(!GDT.getDataStore("UL-Research").data[this.id]){GDT.getDataStore("UL-Research").data[this.id]={};
...
}if(!(i.iconUri.length>0)){i.iconUri=GDT.getRelativePath()+"/img/defaultResearch.png";
}var j={id:i.id,name:i.name,pointsCost:i.pointsCost,canResearch:i.canResearch,iconUri:i.iconUri,description:i.description,targetZone:k,complete:i.complete};
return j;
...
a.getActiveModsString=function(){var d=ModSupport.availableMods;
return activeMods;
};
