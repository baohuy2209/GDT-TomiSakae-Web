if (typeof String.prototype.endsWith == "undefined") {
    ...
    // KHAI BÁO THỦ CÔNG CÁC FILE CSS CẦN TẢI (NẾU CÓ file CSS riêng)
    b.cssFilesToLoad = [
        'mods/UltimateLib/css/checkbox.css', // Thay thế nếu có
        'mods/UltimateLib/css/flot.css',
        'mods/UltimateLib/css/messages.css',
    ];

    // KHÔNG CẦN b.jsFilesToLoad.libraries VÀ b.jsFilesToLoad.libraries3rd NỮA
    // VÌ TẤT CẢ LOGIC JS ĐÃ CÓ TRONG FILE NÀY

    b.init = function () {
        var modEntry = ModSupport.availableMods.find(function (m) { return m.id === "UltimateLib"; });
        if (modEntry) {
            b.mod = modEntry;
        } else {
            console.warn("UltimateLib: Không tìm thấy thông tin mod 'UltimateLib' trong ModSupport.availableMods.");
            b.mod = { id: "UltimateLib", name: "UltimateLib (Fallback)", folder: "mods/UltimateLib/" };
        }

        UltimateLib.Logger.log("UltimateLib: Bắt đầu khởi tạo...");

        // Tải CSS (giữ nguyên)
        if (b.cssFilesToLoad && b.cssFilesToLoad.length > 0) {
            var headElement = $("head");
            $.each(b.cssFilesToLoad, function (index, cssPath) {
                if (cssPath.toLowerCase().endsWith(".css")) {
                    var linkTag = $(document.createElement("link"));
                    linkTag.attr({ rel: "stylesheet", type: "text/css", href: cssPath, media: "all" });
                    headElement.append(linkTag);
                    UltimateLib.Logger.log("UltimateLib: Đã thêm CSS: " + cssPath);
                }
            });
        }

        // KHÔNG CẦN GỌI GDT.loadJs CHO CÁC FILE CON CỦA ULTIMATELIB NỮA

        // Gọi Core.init trực tiếp, vì tất cả các module con đã được định nghĩa
        // trong chính file UltimateLib.js này khi nó được tải.
        if (UltimateLib.Core && UltimateLib.Core.init) {
            UltimateLib.Core.init(); // Core.init sẽ gọi init của các module con khác
        } else {
            UltimateLib.Logger.error("UltimateLib: UltimateLib.Core.init không tồn tại.");
        }

        UltimateLib.Logger.log("UltimateLib: Khởi tạo hoàn tất.");
        // Quan trọng: Báo hiệu rằng mod UltimateLib đã tải xong
        // để ModSupport có thể tiếp tục tải mod tiếp theo.
        // GDT.loadJs (phiên bản tải thẻ script) của file UltimateLib.js này
        // sẽ tự động fire sự kiện GDT.eventKeys.mod.loaded khi nó thực thi xong.
        // Nên không cần fire thủ công ở đây.
    };

    b.initDev = function () {
        console.warn("UltimateLib.initDev() được gọi. Sử dụng logic của init() cho web tĩnh.");
        b.init();
    };
...
UltimateLib.Messages = (function (b) {
    function a() {
    ...
    b.response = function (c) {
        b.show(c, "RESP");
    };
    b.success = function (c) {
        b.show(c, "SUCC");
    };
    b.tutorial = function (c) {
        b.show(c, "TUT");
    };
    b.hint = function (c) {
        b.show(c, "HINT");
    };
    b.show = function (j, r, q, s) {
        var c = "" + j;
        ...
        if (q) {
            ...
            n.attr("title", "Click here to head to the Greenheart Games Forums (http://forum.greenheartgames.com/)");
        } switch (r) {
            case "ERR" || "ERROR": headerText = "Error Alert";
        ...
        if (r === "ERROR" || r === "ERR") {
            k.append('<p class="message">Enabled Mods: ' + DataStore.getValue("enabledMods") + "</p>");
        } l.prependTo(k);
        n.prependTo(k);
        console.log(k);
        l.click(function () {
            var t = b.stack.indexOf(k);
            if (t > -1) {
                b.stack.splice(t, 1);
            } k.remove();
        });
        n.click(function () {
            PlatformShim.openUrlExternal(i);
            k.remove();
        });
        b.stack.push(k);
        if (b.stack && b.stack.length > 0) {
            var o = -(5 * b.stack.length);
...
UltimateLib.Compatibility = (function (d) {
    d.init = function () {
        b();
    };
    function b() {
        LevelCalculator.getFeatureLevel = function (f, h) {
            var i;
            try {
                i = LevelCalculator.getLevel(LevelCalculator.getFeatureExperience(f, h));
            } catch (g) {
                i = 0;
            } return i;
        };
    } function c() {
        var e = Character.prototype._doTraining;
        var f = Character.prototype._doResearch;
        Character.prototype._doTraining = function (i, g) {
            if (GameManager.company.gameLog.length === 0) {
                var h = { releaseWeek: GameManager.company.currentWeek };
...
if (typeof exports !== "undefined") {
    ...
    var V = this, at = V._, aj = {}, ap = Array.prototype, ac = Object.prototype, aq = Function.prototype, al = ap.push, av = ap.slice, z = ap.concat, X = ap.unshift, af = ac.toString, K = ac.hasOwnProperty, aa = ap.forEach, an = ap.map, R = ap.reduce, aw = ap.reduceRight, ah = ap.filter, Z = ap.every, az = ap.some, I = ap.indexOf, ax = ap.lastIndexOf, W = Array.isArray, ar = Object.keys, ay = aq.bind, au = function (a) {
        if (a instanceof au) {
            return a;
    ...
    au.sortBy = function (a, d, b) {
        ...
        return au.pluck(au.map(a, function (f, h, g) {
            return { value: f, index: h, criteria: c.call(b, f, h, g) };
        }).sort(function (f, i) {
            var g = f.criteria, h = i.criteria;
            if (g !== h) {
                if (g > h || g === void 0) {
                    return 1;
                } if (g < h || h === void 0) {
    ...
    au.flatten = function (a, b) {
        ...
        var a = av.call(arguments), d = au.max(au.pluck(a, "length")), b = new Array(d);
        for (var c = 0;
            c < d;
            c++) {
            b[c] = au.pluck(a, "" + c);
    ...
    au.bind = function (d, b) {
        ...
        var b = {};
        return c || (c = au.identity), function () {
        ...
        var d, b;
        return function () {
            var h = this, g = arguments, i = function () {
        ...
        var c = {}, b = z.apply(ap, av.call(arguments, 1));
        return ak(b, function (d) {
            d in a && (c[d] = a[d]);
        }), c;
    }, au.omit = function (a) {
        var d = {}, c = z.apply(ap, av.call(arguments, 1));
        for (var b in a) {
    ...
    var ai = function (g, v, k, m) {
        ...
        if (j != af.call(v)) {
            ...
            case "[object String]": return g == String(v);
        ...
        if (j == "[object Array]") {
            ...
            var b = g.constructor, h = v.constructor;
...
(function () {
    var D = "0.4.7", a = window.jQuery || window.$ || (window.$ = {}), C = {
    ...
    } var v = { __jstorage_meta: { CRC32: {} } }, x = { jStorage: "{}" }, w = null, y = 0, b = false, n = {}, l = false, m = 0, r = {}, q = +new Date(), A, B = {
        isXML: function (G) {
    ...
    function j() {
        ...
        var H = C.parse(C.stringify(v.__jstorage_meta.CRC32)), G;
        s();
        ...
        if (F == "flushed") {
            ...
            G++) {
            if (n[K[G]]) {
                for (var H = 0, I = n[K[G]].length;
                    H < I;
                    H++) {
                    n[K[G]][H](K[G], F);
                }
            } if (n["*"]) {
                for (var H = 0, I = n["*"].length;
                    H < I;
                    H++) {
                    n["*"][H](K[G], F);
                }
        ...
        var H, J, L, G, K = Infinity, F = false, I = [];
        clearTimeout(A);
        ...
        while (I >= 4) {
            ...
            case 3: F ^= (K.charCodeAt(G + 2) & 255) << 16;
    ...
    var d = function (h) {
        function f(l, m, j, i, n, o) {
                ...
                p.onreadystatechange = function () {
                    if (this.readyState == 4) {
                        if (this.status >= 200 && this.status < 300 || this.status === 304) {
                            ...
                            i({ path: m, request: this, error: this.status });
                        }
                    }
                };
            ...
            (function j() {
                f("GET", k, null, function (m, p, q) {
                    ...
                    var n = (q.getResponseHeader("link") || "").split(/\s*,\s*/g), o = a.find(n, function (r) {
        ...
        d.Repository = function (j) {
            ...
            this.updateTree = function (p, t, q, r) {
                var s = { base_tree: p, tree: [{ path: t, mode: "100644", type: "blob", sha: q }] };
            ...
            this.postTree = function (q, p) {
                f("POST", l + "/git/trees", { tree: q }, function (r, s) {
                    if (r) {
                        return p(r);
                    } p(null, s.sha);
                });
            ...
            this.commit = function (s, t, r, p) {
                var q = { message: r, author: { name: j.username }, parents: [s], tree: t };
            ...
            this.updateHead = function (r, q, p) {
                f("PATCH", l + "/git/refs/heads/" + r, { sha: q }, function (s, t) {
                    p(s);
                });
            };
        ...
        this.getIssues = function (j, i) {
            return new d.Issue({ user: j, repo: i });
        };
        this.getRepo = function (j, i) {
            return new d.Repository({ user: j, name: i });
        };
        this.getUser = function () {
            return new d.User();
        };
        this.getGist = function (i) {
            return new d.Gist({ id: i });
        };
    };
    if (typeof exports !== "undefined") {
        module.exports = d;
...
UltimateLib.Achievements = (function (b) {
    ...
    var c = {
        id: "UltimateLib", title: "UltimateLib", description: "Played a game with UltimateLib installed.", isAchieved: function (d) {
            return true;
...
UltimateLib.Booths = (function (a) {
    UltimateLib.Logger.log("UltimateLib.Booths loading...");
    a.init = function () { };
...
UltimateLib.Dialog = (function (a) {
    ...
    a.clearPoweredBy = function (b) {
        $(".ultimatelib-dialog-poweredby", b).remove();
    };
    a.addPoweredBy = function (b) {
        var d = $(document.createElement("div"));
    ...
    c.Staff = (function (k) {
        ...
        function g(m, n, q) {
            ...
            switch (n) {
                case "none": o = l;
                    break;
                case "id": o = l.sort(function (p, q) {
                    return p.id > q.id ? 1 : -1;
                });
                    break;
                case "name": o = l.sort(function (p, q) {
                    return p.name.toLowerCase() > q.name.toLowerCase() ? 1 : -1;
                });
                    break;
                case "sex": o = l.sort(function (p, q) {
                    return p.sex == q.sex ? 1 : -1;
                });
                    break;
                case "salary": o = l.sort(function (p, q) {
                    return p.salary > q.salary ? 1 : -1;
                });
                    break;
                case "experience": o = l.sort(function (p, q) {
                    return p.experience > q.experience ? 1 : -1;
                });
                    break;
                case "speed": o = l.sort(function (p, q) {
                    return p.speedFactor > q.speedFactor ? 1 : -1;
                });
                    break;
                case "technology": o = l.sort(function (p, q) {
                    return p.techFactor > q.techFactor ? 1 : -1;
                });
                    break;
                case "research": o = l.sort(function (p, q) {
                    return p.researchFactor > q.researchFactor ? 1 : -1;
                });
                    break;
                case "design": o = l.sort(function (p, q) {
                    return p.designFactor > q.designFactor ? 1 : -1;
                });
                    break;
                case "quality": o = l.sort(function (p, q) {
                    return p.qualityFactor > q.qualityFactor ? 1 : -1;
                });
                    break;
                case "efficiency": o = l.sort(function (p, q) {
...
UltimateLib.Marketing = (function (a) {
    UltimateLib.Logger.log("UltimateLib.Marketing loading...");
    a.init = function () { };
    ...
    a.showSimpleNotification = function (b, e, d) {
        if (GameManager.company) {
            if (d) {
                ...
                if (!c) {
                    a.Items.push({ id: "Custom_" + b, name: "{" + b + "}", enabled: true, asOverlay: d });
                } else {
                    c.asOverlay = d;
                }
...
UltimateLib.PopupMenu = (function (c) {
    ...
    c.update = function () {
        ...
        var d = function (e, f, g, i) {
            $.each(a, function (h, j) {
                f.push({
                    label: j.label.localize(), action: function () {
                        Sound.click();
                        ...
                        k.scrollTop();
                        k.gdDialog({
                            popout: !0, close: !0, onClose: function () {
                                if (j.pause) {
...
UltimateLib.Reputation = (function (a) {
    UltimateLib.Logger.log("UltimateLib.Reputation loading...");
    a.init = function () { };
    a.Rating = (function (d) {
        var b = GameManager.company;
        d.cash = function (e) {
            var c = e ? e.cash : b.cash;
            var f = 0;
            switch (c) {
                case c > 10000000000: f = 5;
                    break;
        ...
        d.games = function (c) {
            var h = c ? c.gameLog : b.gameLog;
            if (h.length > 0) {
                var j = h.length;
                var g = 0;
                var e = 9;
                var f = $.grep(h, function (i, k) {
                    UltimateLib.Utils.compare(i.score, e);
                });
                f = f.length;
                while ((UltimateLib.Utils.percentage(f, j, true) >= 50) === false) { }
            } return;
        };
        d.sales = function () { };
        d.profit = function () { };
    ...
    function a(i, k) {
        if (k === 0) {
            ...
            i.complete = function (l) {
                GDT.getDataStore("UL-Research").data[i.id].complete = true;
                f(l);
            };
            i.canResearch = function (l) {
                var m = true;
                try {
                    if (GDT.getDataStore("UL-Research").data[i.id].complete === true) {
                        m = false;
                    }
                } catch (n) {
                    GDT.getDataStore("UL-Research").data[i.id].complete = false;
                } return m && g(l);
            };
            ...
            i.iconUri = "./mods/UltimateLib/img/defaultResearch.png";
    ...
    b.write = function (f, c, g) {
        if (!$.jStorage.storageAvailable()) {
            ...
            if (typeof g !== undefined) {
                $.jStorage.set("UltimateLib.Storage." + f, c, { TTL: g });
            } else {
...
UltimateLib.Stats = (function (b) {
    ...
    b.renderGameSalesStats = function (e, c) {
        ...
        var d = $.plot(c.el, [c.data], {
            lines: { show: true, fill: true }, points: { show: true, fillColor: "yellow" }, xaxis: {
                tickSize: 1, tickFormatter: function (g, f) {
                    return parseInt(g);
                }
            }, yaxis: {
                position: "left", tickFormatter: function (g, f) {
        ...
        c.bind("plothover", function (f, i, h) {
            ...
            if (h) {
                var j = parseInt(h.datapoint[0].toFixed(2)), k = UI.getShortNumberString(h.datapoint[1].toFixed(2));
...
UltimateLib.Utils = (function (a) {
    ...
    a.devStats = function () {
        var d = function (h, j) {
            ...
            var g = "";
            switch (j) {
                case "h1": f = "background: #333; color: #bada55; font-weight:bold;";
                    h = "--- " + h + " ---";
...
UltimateLib.Visuals = (function (a) {
    ...
    a.Tweaks = (function (c) {
        ...
        c.setScrollBar = function (f) {
            ...
            switch (f) {
                case 1: g.append("::-webkit-scrollbar { width: 12px; }");
        ...
        c.setFancyGrads = function (f) {
            ...
            switch (f) {
                case 1: g.append(".feature1 .ui-slider-range, .featurePreview1 { background: linear-gradient(to bottom,  #a90329 0%,#8f0222 44%,#6d0019 100%); }");
        ...
        c.setWatermarks = function (f, h) {
            ...
            switch (f) {
                case "slider-all-img": $("#selectFeatureMenuTemplate").find(".focusSliderWrapper").prepend('<img id="allsliderimg" class="ul-vt-slider-img" src="' + h + '"/>');
                    break;
